...
import {KikobaLoanApproverData, RegionData} from "../../../interfaces";
...
export const KikobaLoanApproversPage: React.FC<Props> = (props: Props) => {
  const authProvider = useActiveAuthProvider();
  const {data: user} = useGetIdentity({
    v3LegacyAuthProviderCompatible: Boolean(authProvider?.isLegacy),
  });

  const [approver, setApprover] = useState<KikobaLoanApproverData>();
  const [approvers, setApprovers] = useState<KikobaLoanApproverData[]>([]);
  ...
  const getApprovers = async (
    ...
    const {data} = await simpleRestProvider.custom!<RegionData | any>({
      url: `${configs.apiUrl}/kikoba/${id}/loans/approvers?page=${start}&limit=${limit}&q=${key}`,
      method: "get",
    })
      .then((res) => {
        return res;
